...
def dydt(y, t):
    ...
    state_e1 = np.zeros((Nv, 2 * Nx + 1), dtype="complex128")
    state_e2 = np.zeros((Nv, 2 * Nx + 1), dtype="complex128")
    state_i = np.zeros((Nv, 2 * Nx + 1), dtype="complex128")
    state_i[0, Nx] = np.sqrt(1 / alpha_i)

    for jj in range(Nv):
        state_e1[jj, :] = y[jj * (2 * Nx + 1): (jj + 1) * (2 * Nx + 1)]
        state_e2[jj, :] = y[Nv * (2 * Nx + 1) + jj * (2 * Nx + 1): Nv * (2 * Nx + 1) + (jj + 1) * (2 * Nx + 1)]

    E = y[2 * Nv * (2 * Nx + 1): (2*Nv+1) * (2 * Nx + 1)]
    ...
    dydt_[2 * Nv * (2 * Nx + 1): (2*Nv+1) * (2 * Nx + 1)] = ampere_equation_RHS_two_stream(state_e1=state_e1,
                                                                                           state_e2=state_e2,
                                                                                           state_i=state_i,
                                                                                           alpha_e1=alpha_e1,
                                                                                           alpha_e2=alpha_e2,
...
if __name__ == '__main__':
    ...
    # save results
    np.save("data/SW_sqrt/bump_on_tail/ampere/sol_midpoint_u_60", sol_midpoint_u)
    np.save("data/SW_sqrt/bump_on_tail/ampere/sol_midpoint_t_60", t_vec)
