...
def dydt(y, t):
    ...
    # print("closure perp = ", np.abs(np.flip(E).T @ closure_e1))
    # print("mean closure = ", np.abs(state_e1[-1, Nx]))
    ...
    dydt_[-7] = - np.sqrt(Nv/ 2) * L * (np.flip(state_e1[-1, :]).T @ (alpha_e1 * J + q_e1/(m_e1*alpha_e1) * E_conv) @ closure_e1
