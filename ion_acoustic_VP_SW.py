...
def dydt(y, t):
    ...
    for jj in range(Nv):
        ...
        dydt_[Nv * Nx_total + jj * Nx_total: Nv * Nx_total + (jj + 1) * Nx_total][:Nx] = np.flip(
            np.conjugate(dydt_[Nv * Nx_total + jj * Nx_total: Nv * Nx_total + (jj + 1) * Nx_total][Nx + 1:]))
...
if __name__ == '__main__':
    ...
    for ii, kk in enumerate(range(-Nx, Nx + 1)):
        C_0e[ii] = np.trapz(
            y=(1 / (np.sqrt(2 * np.sqrt(np.pi)))) * (1 + epsilon * np.cos(x / 5)) / alpha_e * np.exp(-2 * np.pi * 1j * kk * x / L), x=x,
            dx=x[1] - x[0]) / L
    ...
    # save results
    np.save("data/SW/ion_acoustic/poisson/sol_midpoint_u_10", sol_midpoint_u)
    np.save("data/SW/ion_acoustic/poisson/sol_midpoint_t_10", t_vec)
